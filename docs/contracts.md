...
| `GET /attention/stream` | cookie auth; optional `Last-Event-ID` | SSE invalidations for the caller's account-level attention list. Approval creation and resolution emit `event: attention`; the client refetches `GET /attention`. Frames carry monotonic ids and the stream sends keepalives |
